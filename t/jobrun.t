...
BEGIN {
	...
	-r 't/test-files/jobrun.conf' || croak "could not read jobrun.conf - $!\n";
	-r 't/test-files/jobs.conf' || croak "could not read jobs.conf - $!\n";

}

{
	print "Jobrun Test will appear to hang for 10 seconds"
}
...
my $configFile = 't/test-files/jobrun.conf';
my $jobFile    = 't/test-files/jobs.conf';
...
my $statusType       = 'all';
my $resumable=0;
my $iterationSeconds=1;
my $logfileBase = 'jobrun-logs';
my $logfileSuffix='log';
my $logdir='/tmp/jobrun-logs';
